...
{
	x = min(max((int)x, 0), (int)Device::Width - 1);
	y = min(max((int)y, 0), (int)Device::Height - 1);
	opacity = min(max((int)opacity, 0), 1);
	width += x;
	height += y;
	...
	for (int32 i = 0, x2 = x; i < textLength; i++)
	{
		...
		for (int32 py = 0; py < font->Height; py++)
		{
			for (int32 px = 0, *backBufferPtr = &backBuffer[x2 + (y + py) * Device::Width]; px < characterWidth; px++)
			{
				...
				int32 opacityA = (int)bitmapPtr[(px + characterDimension.X + characterX) + (py + characterY) * (font->Width << 4)];
