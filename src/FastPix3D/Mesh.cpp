...
Mesh* Mesh::FromFile(string path)
{
	...
	if (name !="FastPix3D_Mesh_1.0.2") throw std::runtime_error("Invalid file!");
	...
	for (int32 i = 0; i < mesh->SubsetCount; i++)
	{
		...
		for (int32 a = 0; a < vertexCount; a++)
		{
			...
			subset->Vertices[a].VertexColor = Color((int)file.ReadByte(), (int)file.ReadByte(), (int)file.ReadByte());
